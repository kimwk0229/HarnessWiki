...
description: >
  ...
  (7) 모든 링크 검증, (8) log.md 기록, (9) Skill 도구로 /validate-wiki 실제 호출 후
  그 결과를 포함한 통합 보고. 사용자가 회의 메모나 STT 전사본을 제공할 때마다 이 스킬을 사용하세요.
  CLAUDE.md의 "오퍼레이션 1: 수집" 9단계를 완전 자동화하며, 9단계 자체가 검증 자동 연계를 포함하므로
  ingest 파일 목록만 먼저 보고하고 끝내지 않습니다.
compatibility:
  requires_tools: [Read, Write, Edit, Glob, Grep, Bash, Skill]
---
...
- **생성/갱신된 파일 목록**:
  ...
  - log.md (기록)
- **통합 결과 보고**: ingest 파일 목록 + `/validate-wiki` 검증 판정(PASS/REVIEW/FAIL)을 하나로 합친
  최종 메시지. ingest는 이 통합 보고를 출력해야 종료된 것으로 간주하며, 검증 없이 파일 목록만
  보고하고 끝나지 않습니다.
...
### 9단계: 검증 자동 연계 및 통합 보고 (Auto-chain to Validation + Combined Report)

8단계가 끝나면 아래 순서를 예외 없이 그대로 실행합니다. **중간에 "여기까지 완료했습니다" 식의 보고를
먼저 출력하지 않습니다** — validate-wiki 호출은 보고의 전제조건이며, 보고 뒤에 덧붙이는 부가 단계가
아닙니다.

- **1) 보고 보류**: 8단계 완료 시점에는 파일 목록, 갱신 페이지 목록 등 어떤 결과도 아직 사용자에게
  출력하지 않습니다.
- **2) Skill 도구로 즉시 호출**: 사용자의 추가 명령 입력을 기다리지 않고, 지금 이 턴에서 **Skill
  도구를 사용해 validate-wiki 스킬을 실제로 호출**합니다. 대상 날짜는 1단계에서 이미 확정한
  YYYY-MM-DD를 그대로 사용하며, 날짜를 다시 묻지 않습니다. (`/validate-wiki YYYY-MM-DD`에 대응하는
  실행이지만, 이는 텍스트로 명령어를 적는 것이 아니라 **실제 도구 호출(tool call)**을 의미합니다.)
- **3) 결과 대기**: validate-wiki의 통합 리포트와 최종 판정(PASS/REVIEW/FAIL)을 받을 때까지 기다립니다.
- **4) 통합 보고 (1회만 출력)**: validate-wiki 결과를 받은 후에만 ingest 결과와 validate-wiki 결과를
  하나로 합쳐 다음을 모두 포함한 **단일 메시지**로 보고합니다:
  - 생성된 파일: raw/YYYY-MM-DD_슬러그/, meta.md
  - 갱신된 토픽 페이지 목록
  - 신규 생성된 토픽 페이지 (있으면)
  - decisions.md, action-items.md 갱신 여부
  - 식별된 이슈 (깨진 링크 등)
  - validate-wiki의 최종 판정과 발견사항
  - validate-wiki 3단계가 요구하는 커밋 확인 질문 (예: "검증 결과가 ✅ APPROVED입니다. 커밋할까요?")
    — 이 질문까지 포함해 사용자 응답을 기다리고 종료합니다.

- **rule**: "자동으로 validate-wiki를 실행하겠습니다", "이제 검증을 진행합니다" 등의 문장을 **텍스트로만
  출력하고 실제 도구 호출 없이 턴을 종료하는 것은 규칙 위반**입니다. 실행 의도를 서술하는 것은
  준수(compliance)가 아닙니다. Skill 도구 호출이 실제로 일어나야만 이 단계를 완료한 것으로 인정됩니다.
- **rule**: 1~8단계가 전부 정상 완료된 경우에만 위 절차를 진행합니다. 메타정보 미확정, 필수 파일 저장
  실패, 깨진 링크 미해결 등으로 ingest가 중간에 멈춘 경우 validate-wiki를 호출하지 않고 먼저
  ingest를 완료(또는 사용자에게 확인)합니다.
- **rule**: validate-wiki 호출은 ingest 1회 실행당 정확히 1회만 수행합니다. validate-wiki와
  content-review는 절대 ingest를 다시 호출하지 않으므로 순환 호출(무한 루프) 위험 없음.
- **금지**: validate-wiki의 최종 판정을 통합 보고에 포함하기 전에 1~8단계 재실행 또는 같은 회의
  재-ingest 금지.
- **예외**: 사용자가 이번 요청에서 명시적으로 "검증은 나중에" / "검증하지 마"라고 말한 경우에만
  2)~3)을 생략하고, 4)의 통합 보고 대신 ingest 결과만 보고하며 검증을 생략했다는 사실을 명시적으로
  덧붙입니다.
...
- [ ] validate-wiki를 Skill 도구로 실제 호출(텍스트 언급이 아닌 실제 tool call)하고, 그 결과와
  ingest 파일 목록을 하나로 합친 통합 보고를 사용자에게 제시
...
- `log.md`에 `[INGEST]` 항목 추가
- validate-wiki 호출은 9단계 자체에 포함된 필수 동작이다. ingest 파일 목록을 먼저 보고하고 나서
  연계하는 것이 아니라, Skill 도구로 validate-wiki를 실제로 호출한 뒤 그 결과를 포함해 하나의
  통합 보고로 출력한다 (사용자 명령 대기 없음)
...
- ingest 파일 목록만 먼저 보고하고 validate-wiki 연계를 뒤로 미루거나 생략 금지 (검증 없는 커밋
  유도 금지)
- "자동으로 validate-wiki를 호출하겠습니다" 등의 문장만 출력하고 실제 Skill 도구 호출 없이 턴을
  종료하는 것 금지 (의도를 말하는 것은 실행이 아니다)
