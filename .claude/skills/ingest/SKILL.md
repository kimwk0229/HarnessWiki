...
description: >
  ...
  (7) 모든 링크 검증, (8) log.md 기록, (9) 결과 보고, (10) /validate-wiki 자동 연계.
  사용자가 회의 메모나 STT 전사본을 제공할 때마다 이 스킬을 사용하세요.
  CLAUDE.md의 "오퍼레이션 1: 수집" 9단계를 완전 자동화하고 검증까지 자동 연계합니다.
compatibility:
...
- **rule**: 1~9단계가 전부 정상 완료된 경우에만 체이닝. 메타정보 미확정, 필수 파일 저장 실패, 깨진 링크 미해결 등으로 ingest가 중간에 멈춘 경우 체이닝하지 않고 먼저 ingest를 완료(또는 사용자에게 확인)
- **rule**: 체이닝은 ingest 1회 실행당 정확히 1회만 수행. validate-wiki와 content-review는 절대 ingest를 다시 호출하지 않으므로 순환 호출(무한 루프) 위험 없음
- **금지**: validate-wiki의 최종 판정을 보여주기 전에 1~9단계 재실행 또는 같은 회의 재-ingest 금지
- **예외**: 사용자가 이번 요청에서 명시적으로 "검증은 나중에" / "검증하지 마"라고 말한 경우에만 생략하고, 생략 사실을 9단계 보고에 덧붙임
...
- `log.md`에 `[INGEST]` 항목 추가
- ingest 9단계 완료 즉시 validate-wiki로 자동 연계 (사용자 명령 대기 없음)
...
- ingest 완료 후 validate-wiki 연계 없이 방치 금지 (검증 없는 커밋 유도 금지)
